...
def getRetweet(api, listaInput, lenLista, list_ret, dizionario_tweet, dizionario_retweet, query, start, stop, char):
    ...
    print('scrivo su pickle il dizionario ottenuto' + char)
    with open(
            './pickle/' + query + 'TestAWS/Novembre/tweet' + query + '_' + start + '_' + stop + '_dictionaryReTweet' + char + '.pkl',
            'wb') as output:
        pickle.dump(dizionario_retweet, output, pickle.HIGHEST_PROTOCOL)
    print ('fatto')
    return list_ret
...
def retweetmain(readList, query, start, stop, dizionario_tweet, char):
    ...
    with open('./pickle/' + query + 'TestAWS/Novembre/retweet' + char + query + '_' + start + '_' + stop + '_data.pkl',
              'wb') as output:
...
def main():
    ...
    with open('./pickle/' + query + 'TestAWS/Novembre/tweetBlue' + query + '_' + start + '_' + stop + '_data.pkl',
              'wb') as output:
        pickle.dump(listBlue, output, pickle.HIGHEST_PROTOCOL)

    with open(
            './pickle/' + query + 'TestAWS/Novembre/tweetBlue' + query + '_' + start + '_' + stop + '_dictionaryTweet.pkl',
            'wb') as output:
        pickle.dump(dizionario_tweet_Blue, output, pickle.HIGHEST_PROTOCOL)

    with open('./pickle/' + query + 'TestAWS/Novembre/tweetRed' + query + '_' + start + '_' + stop + '_data.pkl',
              'wb') as output:
        pickle.dump(listRed, output, pickle.HIGHEST_PROTOCOL)

    with open(
            './pickle/' + query + 'TestAWS/Novembre/tweetRed' + query + '_' + start + '_' + stop + '_dictionaryTweet.pkl',
            'wb') as output:
        pickle.dump(dizionario_tweet_Red, output, pickle.HIGHEST_PROTOCOL)

    with open('./pickle/' + query + 'TestAWS/Novembre/tweetYellow' + query + '_' + start + '_' + stop + '_data.pkl',
              'wb') as output:
        pickle.dump(listYellow, output, pickle.HIGHEST_PROTOCOL)

    with open(
            './pickle/' + query + 'TestAWS/Novembre/tweetYellow' + query + '_' + start + '_' + stop + '_dictionaryTweet.pkl',
            'wb') as output:
