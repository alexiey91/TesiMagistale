...
def getRetweet(api, listaInput, lenLista, list_ret, dizionario_tweet, dizionario_retweet, query, start, stop, char):
    for i in listaInput:
        try:
            ...
            print('exception raised, waiting 15 minutes',char)
            print('(until:', dt.datetime.now() + dt.timedelta(minutes=15), ')')
            print("check list len" + str(len(list_ret)))
            print("check list len dopo scrittura file" + str(len(list_ret)))
            time.sleep(15 * 60)
    ...
    print('scrivo su pickle il dizionario ottenuto' + char)
    with open(
            './pickle/' + query + 'TestAWS/Gennaio/tweet' + query + '_' + start + '_' + stop + '_dictionaryReTweet' + char + '.pkl',
            'wb') as output:
        pickle.dump(dizionario_retweet, output, pickle.HIGHEST_PROTOCOL)
    print ('fatto')
    return list_ret
...
def retweetmain(readList, query, start, stop, dizionario_tweet, char):
    ...
    with open('./pickle/' + query + 'TestAWS/Gennaio/retweet' + char + query + '_' + start + '_' + stop + '_data.pkl',
              'wb') as output:
...
def main():
    ...
    if not os.path.exists('pickle/' + query + 'TestAWS/Gennaio/'):
        os.makedirs('pickle/' + query + 'TestAWS/Gennaio/')
    ...
    with open('./pickle/' + query + 'TestAWS/Gennaio/tweetBlue' + query + '_' + start + '_' + stop + '_data.pkl',
              'wb') as output:
        pickle.dump(listBlue, output, pickle.HIGHEST_PROTOCOL)

    with open(
            './pickle/' + query + 'TestAWS/Gennaio/tweetBlue' + query + '_' + start + '_' + stop + '_dictionaryTweet.pkl',
            'wb') as output:
        pickle.dump(dizionario_tweet_Blue, output, pickle.HIGHEST_PROTOCOL)

    with open('./pickle/' + query + 'TestAWS/Gennaio/tweetRed' + query + '_' + start + '_' + stop + '_data.pkl',
              'wb') as output:
        pickle.dump(listRed, output, pickle.HIGHEST_PROTOCOL)

    with open(
            './pickle/' + query + 'TestAWS/Gennaio/tweetRed' + query + '_' + start + '_' + stop + '_dictionaryTweet.pkl',
            'wb') as output:
        pickle.dump(dizionario_tweet_Red, output, pickle.HIGHEST_PROTOCOL)

    with open('./pickle/' + query + 'TestAWS/Gennaio/tweetYellow' + query + '_' + start + '_' + stop + '_data.pkl',
              'wb') as output:
        pickle.dump(listYellow, output, pickle.HIGHEST_PROTOCOL)

    with open(
            './pickle/' + query + 'TestAWS/Gennaio/tweetYellow' + query + '_' + start + '_' + stop + '_dictionaryTweet.pkl',
            'wb') as output:
    ...
    with open('./pickle/' + query + 'TestAWS/Gennaio/Finito' + str(dt.datetime.now()), "wb") as output:
