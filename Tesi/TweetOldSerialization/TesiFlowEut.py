...
def getRetweet(api, listaInput, lenLista, list_ret, dizionario_tweet, dizionario_retweet, query, start, stop, char):
    ...
    print('scrivo su pickle il dizionario ottenuto' + char)
    with open(
            './pickle/' + query + 'TestAWS/Ottobre/tweet' + query + '_' + start + '_' + stop + '_dictionaryReTweet' + char + '.pkl',
            'wb') as output:
        pickle.dump(dizionario_retweet, output, pickle.HIGHEST_PROTOCOL)
    print ('fatto')
    return list_ret
...
def retweetmain(readList, query, start, stop, dizionario_tweet, char):
    ...
    with open('./pickle/' + query + 'TestAWS/Ottobre/retweet' + char + query + '_' + start + '_' + stop + '_data.pkl',
              'wb') as output:
...
def main():
    ...
    if not os.path.exists('pickle/' + query + 'TestAWS/Ottobre/'):
        os.makedirs('pickle/' + query + 'TestAWS/Ottobre/')
    ...
    with open('./pickle/' + query + 'TestAWS/Ottobre/tweetBlue' + query + '_' + start + '_' + stop + '_data.pkl',
              'wb') as output:
        pickle.dump(listBlue, output, pickle.HIGHEST_PROTOCOL)

    with open(
            './pickle/' + query + 'TestAWS/Ottobre/tweetBlue' + query + '_' + start + '_' + stop + '_dictionaryTweet.pkl',
            'wb') as output:
        pickle.dump(dizionario_tweet_Blue, output, pickle.HIGHEST_PROTOCOL)

    with open('./pickle/' + query + 'TestAWS/Ottobre/tweetRed' + query + '_' + start + '_' + stop + '_data.pkl',
              'wb') as output:
        pickle.dump(listRed, output, pickle.HIGHEST_PROTOCOL)

    with open(
            './pickle/' + query + 'TestAWS/Ottobre/tweetRed' + query + '_' + start + '_' + stop + '_dictionaryTweet.pkl',
            'wb') as output:
        pickle.dump(dizionario_tweet_Red, output, pickle.HIGHEST_PROTOCOL)

    with open('./pickle/' + query + 'TestAWS/Ottobre/tweetYellow' + query + '_' + start + '_' + stop + '_data.pkl',
              'wb') as output:
        pickle.dump(listYellow, output, pickle.HIGHEST_PROTOCOL)

    with open(
            './pickle/' + query + 'TestAWS/Ottobre/tweetYellow' + query + '_' + start + '_' + stop + '_dictionaryTweet.pkl',
            'wb') as output:
    ...
    with open('./pickle/' + query + 'TestAWS/Ottobre/Finito' + str(dt.datetime.now()), "wb") as output:
